...
/**
  ...
  * @retval None
  */
void LCD_DrawChar(uint16_t Xpos, uint16_t Ypos, const uint16_t *c,uint16_t PenColor, uint16_t BackColor)
{
  ...
  for(index = 0; index < LCD_Currentfonts->Height; index++)
  {
    ...
    for(counter = 0; counter < LCD_Currentfonts->Width; counter++)
    {
        ...
        (((c[index] & (0x1 << counter)) == 0x00)&&(LCD_Currentfonts->Width > 12 )))
      {
         ...
         *(__IO uint16_t*) (CurrentFrameBuffer + (2*Xaddress) + xpos) =BackColor;
      }
      else
      {
          /* Write data value to all SDRAM memory */
         *(__IO uint16_t*) (CurrentFrameBuffer + (2*Xaddress) + xpos) = PenColor;         
      }
      Xaddress++;
    }
...
/**
  ...
  * @retval None
  */
void LCD_DisplayChar(uint16_t x, uint16_t y, uint8_t Ascii,uint16_t PenColor, uint16_t BackColor)
{
  Ascii -= 32;

  LCD_DrawChar(x, y, &LCD_Currentfonts->table[Ascii * LCD_Currentfonts->Height],PenColor, BackColor);
}
...
/**
  ...
  * @retval None
  */
void LCD_DisplayString(uint16_t x,uint16_t y, uint8_t *ptr,uint16_t PenColor, uint16_t BackColor)
{  
  /* Send the string character by character on lCD */
  while ((y < LCD_PIXEL_WIDTH) && ((*ptr != 0) & (((y + LCD_Currentfonts->Width) & 0xFFFF) >= LCD_Currentfonts->Width)))
  {
    /* Display one character on LCD */
    LCD_DisplayChar(y, x, *ptr,PenColor,BackColor);
...
/**
  ...
  * @param  Xpos: specifies the X bottom left position from 0 to 480.
  * @param  Ypos: specifies the Y bottom left position from 0 to 272.
  * @param  Height: display window height, can be a value from 0 to 272.
  * @param  Width: display window width, can be a value from 0 to 480.
  * @retval None
  */
...
/**
  ...
  * @retval None
  */
void LCD_DrawChanese(uint16_t x, uint16_t y, const uint8_t *c, uint16_t PenColor, uint16_t BackColor)
{
  uint32_t index = 0, counter = 0, xpos =0;
  uint32_t  Xaddress = 0;
...
/**
  ...
  * @param  *c: 指向显示缓冲区
  * @param  PenColor,BackColor:字符颜色,背景颜色
  * @param  size:size16x16 , size24x24
  * @retval None
  */
void LCD_DisplayChanese(uint16_t x, uint16_t y, uint8_t* hz,uint16_t PenColor, uint16_t BackColor,FONT_SIZE size)
{
  uint16_t i=0;
	FNT_GB16 *ptGb16 = (FNT_GB16 *)Chinese16x16_Table;		  //ptGb16指向GBHZ_16
	FNT_GB24 *ptGb24 = (FNT_GB24 *)Chinese24x24_Table;		  //ptGb24指向GBHZ_24
	switch(size){
		case size16x16:
			while(ptGb16[i].Index[0] > 0x80)
	    {
		    ...
		    {
			    LCD_DrawChanese(x, y, &ptGb16[i].Msk[0],PenColor, BackColor);
		    }
		   i++;
		   if(i > (sizeof((FNT_GB16 *)Chinese16x16_Table) / sizeof(FNT_GB16) - 1))  //搜索下标约束 
			break;
	   }	
	  break;
		case size24x24:
			while(ptGb24[i].Index[0] > 0x80)
	     {
		    ...
		    {
			    LCD_DrawChanese(x, y, &ptGb24[i].Msk[0],PenColor, BackColor);
		    }
		      i++;
    ...
    while (*s!='\0') {
			...
			if(*s<0x80){
				switch(size)
				{
				 ...
				 LCD_DisplayChar(y,x,*s,PenColor,BackColor);			
				 x+=LCD_Currentfonts->Width;
				if(x>LCD_PIXEL_WIDTH-LCD_Currentfonts->Width) //换行
         {
			  ...
			  LCD_DisplayChanese( y,x, s++,PenColor, BackColor,size);
			  x += LCD_Currentfonts->Width;	
				if(x>LCD_PIXEL_WIDTH-LCD_Currentfonts->Width)     //换行
         {
					 y+=LCD_Currentfonts->Height;
...
{
	static	u16 tx=0,ty=0;  uint32_t index = 0;
	u16 *bitmap = (u16 *)pic;
  unsigned short temp;
	//LCD_WindowMax(StartX, StartY, Xend, Yend);	//设置图片显示窗口大小
//	LCD_SetDisplayWindow(StartX, StartY, Height, Height);
	//LCD_SetCursor(StartX,StartY);
	//LCD_WriteRAM_Prepare();
//	LCD_SetCursor(StartX,StartY);
//  /* erase memory */
//  for (index = StartX; index < Height*Width; index++)
//  {
//    *(__IO uint16_t*)(CurrentFrameBuffer+ (StartX*StartY)+ (2*index)) = *bitmap++;
//  } 
	
	
	
	  for(tx=0;tx<Height;tx++)//这里将数据送到屏幕,边读边送
	  {							
			for(ty=0;ty<Width;ty++)
			{	
				PutPixelColor(StartX+ty,StartY+tx,*bitmap++);		
			}				
		}

}
extern unsigned char gImage_cloudy[5832];
#include "stdio.h"

uint32_t DMA2D_OutAdd;//=(uint32_t)gImage_cloudy;
...
void DMA2D_M2M_Config(uint16_t Xpos, uint16_t Ypos, uint16_t Width, uint16_t Height)
{
  ...
  printf("\r\n0x%x\r\n",DMA2D_OutAdd);
    ...
    DMA2D_InitStruct.DMA2D_OutputGreen = 100;
    DMA2D_InitStruct.DMA2D_OutputBlue = 100;
    DMA2D_InitStruct.DMA2D_OutputRed = 100;
